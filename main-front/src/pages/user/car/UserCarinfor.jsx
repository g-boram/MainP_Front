...
export default function UserCarinfor() {
  ...
  useEffect(() => {
    if (!carNumber) return;
    axios
      .get(`http://localhost:8081/details/${carNumber}`)
      .then((response) => {
  ...
  return (
    <CarListContainer>
      ...
      <DescArea>
        <SubText2>
          ·시세는 최근 3개월 내 시장가격이 반영된 차량의 견적가 기준으로
          반영됩니다
        </SubText2>
        <SubText2>
          ·차량의 상태에 따라 실제 견적 가격은 달라질 수 있습니다.
        </SubText2>
      </DescArea>
      <LinkStyle to={`/carsell`} state={carNumber}>
        <FormBtn>계속 진행하기</FormBtn>
      </LinkStyle>
